...
const picks = [
  ['Heaven or Las Vegas','Cocteau Twins','Dreamy textures and bright melodies echo artists already in your rotation.','Lightly familiar','Related artist',['#d15648','#f2bb63']],
  ['Promises','Floating Points, Pharoah Sanders','A patient, immersive listen connecting your ambient and jazz interests.','New to you','Taste crossover',['#73513e','#ddd0a8']],
  ['Titanic Rising','Weyes Blood','Expansive songwriting with the warm, cinematic detail you return to often.','Somewhat familiar','Listening pattern',['#245b67','#6fb9b4']],
  ['Afrique Victime','Mdou Moctar','Kinetic guitar music with an adventurous edge that fits your recent listening.','New to you','Genre connection',['#ba623f','#352b41']],
  ['Dots and Loops','Stereolab','Intricate rhythms and playful production bridge several of your favorite sounds.','Lightly familiar','Deep cut',['#55776b','#d9c8a5']],
  ['The Campfire Headphase','Boards of Canada','Soft-focus electronic details align with the quieter side of your library.','New to you','Mood match',['#9d7551','#d8b66b']],
  ['Sometimes I Might Be Introvert','Little Simz','Bold arrangements and precise storytelling match your most replayed records.','Somewhat familiar','Taste crossover',['#a74835','#e2b676']],
  ['Ants From Up There','Black Country, New Road','Cathartic ensemble rock for your taste in ambitious album-length journeys.','New to you','Related artist',['#5b6b79','#c2d4de']],
  ['Lahai','Sampha','Intimate vocals and tactile production sit naturally beside your soulful favorites.','Lightly familiar','Listening pattern',['#92563d','#d8a060']],
  ['Blue Rev','Alvvays','Sharp hooks under layers of guitar connect to your melodic indie listening.','New to you','Genre connection',['#476287','#b1c5dc']],
...
function Header({ authenticated, accountName, previewing, logout, deleteAccount, home }) {
  return <header><button className="brand" onClick={home}><i>▥</i> LINER NOTES</button><div className="header-actions">{previewing&&<button className="link" onClick={home}>← Back to home</button>}{authenticated&&<details className="account-menu"><summary><span className="avatar" aria-hidden="true"><i/></span><span>{accountName}</span><b aria-hidden="true">⌄</b></summary><div className="account-dropdown"><button onClick={logout}>Log out <span>↗</span></button><button className="delete-option" onClick={deleteAccount}>Delete account &amp; data <span>×</span></button></div></details>}</div></header>
}

function Landing({ auth, preview, load, removeAccount, confirming, setConfirming, accountBusy, accountError }) {
  return <main className="landing"><section><p className="eyebrow">Your next favorite record is already connected</p><h1>Find the albums<br/><em>between the lines.</em></h1><p className="lede">Thoughtful album recommendations shaped by what you actually listen to—not what everyone else is playing.</p>{auth==='authenticated'?<><p className="welcome">Spotify connected. Where would you like to go?</p><div className="actions signed-in"><button className="button primary" onClick={load}>Load recommendations →</button><button className="button quiet" onClick={preview}>Preview the experience →</button></div>{confirming&&<div className="delete-confirm" role="alertdialog" aria-labelledby="delete-title"><strong id="delete-title">Delete your account and data?</strong><p>This permanently removes your saved Spotify credentials, sessions, and cached recommendation profile from Liner Notes. It cannot be undone.</p>{accountError&&<p className="account-error">{accountError}</p>}<div><button className="button danger" onClick={removeAccount} disabled={accountBusy}>{accountBusy?'Deleting…':'Yes, delete everything'}</button><button className="button quiet" onClick={()=>setConfirming(false)} disabled={accountBusy}>Cancel</button></div></div>}</>:auth==='checking'?<button className="button primary" disabled>Checking Spotify connection…</button>:<div className="actions"><a className="button primary" href={`${API}/api/auth/login`}>● Connect with Spotify ↗</a><button className="button quiet" onClick={preview}>Preview the experience →</button></div>}<p className="privacy">We only read the listening data needed to shape your recommendations.</p></section><aside className="stack"><div/><div/><div/></aside></main>
}

function Loading() {
  return <main className="state"><div className="vinyl"><i/></div><p className="eyebrow">Reading your listening history</p><h1>Pulling your taste<br/>into focus.</h1><p>We’re tracing artists, albums, and listening patterns.<br/>This can take a moment the first time.</p><span className="progress"><i/></span></main>
}
...
function Card({ album, number, choice, choose }) {
  ...
  return <article className="card"><Cover album={album} number={number}/><div><p className="side">SIDE {String(number).padStart(2,'0')}</p><h2>{album.album_name}</h2><p className="artist">{album.artist_name}</p><p className="reason">{album.reason}</p><div className="signals"><span><small>FAMILIARITY</small>{album.familiarity_label}</span><span><small>DISCOVERY PATH</small>{album.discovery||'Artist connection'}</span></div><div className="card-actions"><a className={`spotify ${album.spotify_url?'':'disabled'}`} href={album.spotify_url||undefined} target="_blank" rel="noreferrer">Open in Spotify ↗</a><div className="feedback">{options.map(([value,icon,label])=><button key={value} className={choice===value?'selected':''} onClick={()=>choose(value)} title={label} aria-label={label} aria-pressed={choice===value}><span>{icon}</span><b>{label}</b></button>)}</div></div></div></article>
}

function Results({ albums, demo, choices, choose, more, busy }) {
  return <main className="results">{demo&&<div className="demo">Preview mode — these picks are placeholders</div>}<section className="intro"><p className="eyebrow">A fresh stack for you</p><h1>Five albums.<br/><em>Worth your time.</em></h1><p>Picked from the edges of your listening history—familiar enough to click, far enough to surprise you.</p></section><div className="grid">{albums.map((a,i)=><Card key={`${a.artist_name}-${a.album_name}`} album={a} number={i+1} choice={choices[a.album_name]} choose={v=>choose(a.album_name,v)}/>)}</div><section className="more"><p className="eyebrow">Keep digging</p><h2>There’s always another record.</h2><button className="button primary" onClick={more} disabled={busy}>{busy?'Finding five more…':'Discover 5 more →'}</button></section></main>
}
